...
*/
    ...
    private void checkConsumerIncompleteAnomaly(
        ...
        try {
            // merge errors
            Set<String> combinedErrors = new HashSet<>(errors);
            combinedErrors.addAll(checkNullAttributesForFile(consumer));

            String distinctErrors = String.join(", ", combinedErrors);

            // update consistency
            consumer.setIsConsistent(combinedErrors.isEmpty());
            consumer.setConsistentOn(combinedErrors.isEmpty() ? "N/A" :
                    LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
            consumerRepository.save(consumer);

            // anomaly type
            AnomalyType anomalyType =
                    anomalyTypeRepository.findFirstByName("Incomplete Data");

            // existing anomaly?
            Long anomalyId =
                    consumerAnomalyRepository.findFirstAnomalyId(consumer.getId(), anomalyType.getId());

            Anomaly anomaly;

            if (anomalyId != null) {
                anomaly = anomalyRepository.findById(anomalyId).orElseThrow();

                // update note
                anomaly.setNote("Missing Mandatory Fields: " + distinctErrors);

                // status transition
                if (!consumer.getIsConsistent()) {
                ...
                anomaly.setUpdateBy(user.getFirstName());
                ...
                // formatted ID
                anomaly.setAnomalyFormattedId(generateFormattedAnomalyId(
                        consumer.getServiceProvider(),
                        anomaly.getId(),
                        anomaly.getReportedOn()
                ));
                anomalyRepository.save(anomaly);
                ...
                anomalyTrackingRepository.save(
                        new AnomalyTracking(
                                ...
                                "", user.getFirstName(), anomaly.getUpdatedOn()
                        )
                );
            }

            // safe linking
            safeLinkConsumerToAnomaly(consumer, anomaly, anomaly.getNote());

        } catch (Exception e) {
            log.error("FAILED checkConsumerIncompleteAnomaly | consumer={} error={}",
                    consumer.getId(), e.getMessage(), e);
            em.clear();
        }
    }
   ...
   /* private AnomalyStatus resolveAnomalyStatus(Anomaly anomaly) {
    ...
    @Transactional
    public void safeLinkConsumerToAnomaly(Consumer consumer, Anomaly anomaly, String note) {

        if (consumer == null || anomaly == null) return;

        try {
            // 🔍 Try to fetch existing link
            Optional<ConsumerAnomaly> existingOpt =
                    consumerAnomalyRepository.findByConsumer_IdAndAnomaly_Id(
                            consumer.getId(), anomaly.getId()
                    );

            // -------------------------------------------------------
            // CASE 1 – Link already exists → update note if required
            // -------------------------------------------------------
            if (existingOpt.isPresent()) {
                ConsumerAnomaly existing = existingOpt.get();

                if (!Objects.equals(existing.getNotes(), note)) {
            ...
            // -------------------------------------------------------
            // CASE 2 – No link exists → create new one
            // -------------------------------------------------------
            ...
            log.info("Linked consumer {} to anomaly {} (note={})",
                    consumer.getId(), anomaly.getId(), note);
        }

        // -------------------------------------------------------
        // Duplicate protection (race-condition safety)
        // -------------------------------------------------------
        catch (DataIntegrityViolationException ex) {
            log.warn("Duplicate anomaly link detected for consumer={} anomaly={}, ignoring.",
                    consumer.getId(), anomaly.getId());

            // no em.clear() → this prevents TransactionRequiredException
        }
        ...
        // -------------------------------------------------------
            ...
            log.error("safeLinkConsumerToAnomaly FAILED consumer={} anomaly={} note={} err={}",
                    consumer.getId(), anomaly.getId(), note, ex.getMessage(), ex);
        }
    }
    ...
    private Consumer resolvedAndDeleteExceedingConsumers(Consumer consumer, Boolean flag, User user) {
        ...
        // 🔹 Fetch all consumers with same ID Type + Number + SP
        ...
        // 🔹 Find existing anomalies for these consumers
        List<Long> consumerAnomalies =
                consumerAnomalyRepository.findAnomaliesIdByConsumerAndAnomalyTypeId(consumerIds, anomalyType.getId());
        ...
        if (!consumerAnomalies.isEmpty()) {
            ...
            if (anomaly != null) {
                ...
                // ✅ CASE 1: Already tagged & previously under resolution (status = 4 or 5)
                if (anomaly.getStatus().getCode() == 4 || anomaly.getStatus().getCode() == 5) {

                    // 🔹 Ensure all consumers linked to this anomaly are consistent
                    duplicateConsumers.forEach(c -> {
                        c.setIsConsistent(true);
                        c.setConsistentOn(LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
                    });
                    ...
                    if (newStatus != anomaly.getStatus()) {
                        ...
                        anomalyTrackingRepository.save(
                                new AnomalyTracking(
                                        ...
                                        new Date(),
                                        newStatus,
                                        "Automatically resolved (Exceeding Threshold)",
                                        user.getFirstName() + " " + user.getLastName(),
                                        anomaly.getUpdatedOn()
                                )
                        );
                ...
                // ✅ CASE 2: Active anomalies (status 1–3)
                if (anomaly.getStatus().getCode() == 1 ||
                    ...
                    // Mark as inconsistent until resolved
                    ...
                    log.info("⚠️ Active Exceeding anomaly {} retained in status {} for consumer {}",
                            anomaly.getId(), anomaly.getStatus(), consumer.getMsisdn());
                }
            }
        }

        // ✅ Soft-delete older duplicates if count > 2
        ...
        log.info("resolvedAndDeleteExceedingConsumers → consumer={} consistent={} statusUpdateChecked",
                consumer.getMsisdn(), consumer.getIsConsistent());
    ...
    public AnomalyStatus checkAnomalies(List<ConsumerAnomaly> links, User user, ServiceProvider sp) {

        if (links == null || links.isEmpty()) return null;

        boolean allConsistent = links.stream()
                .allMatch(ca -> Boolean.TRUE.equals(ca.getConsumer().getIsConsistent()));

        boolean allInconsistent = links.stream()
                .allMatch(ca -> Boolean.FALSE.equals(ca.getConsumer().getIsConsistent()));

        if (allConsistent) {
            return AnomalyStatus.RESOLVED_FULLY;  // 6
        }
        else if (!allConsistent && !allInconsistent) {
            return AnomalyStatus.RESOLVED_PARTIALLY; // 5
        }
        else {
            return AnomalyStatus.RESOLUTION_SUBMITTED; // 4
        }
    }
    ...
    public void updateAnomalyStatusForConsumers(List<Consumer> newConsumers,
        ...
        // 🔥 1) Take snapshot of OLD consumers BEFORE update
        Map<Long, Consumer> oldConsumerSnapshot =
                consumerRepository.findByServiceProvider_Id(sp.getId())
                        .stream()
        ...
        for (Map.Entry<Long, List<ConsumerAnomaly>> entry : anomaliesById.entrySet()) {
            ...
            // Compute new status with OLD snapshot included
            AnomalyStatus newStatus = computeGroupStatus(
                    anomaly, links, user, sp, oldConsumerSnapshot
            );
    ...
    /**
     * Compute anomaly status for threshold, duplicate, missing mandatory.
     */
    private AnomalyStatus computeGroupStatus(Anomaly anomaly,
                                             List<ConsumerAnomaly> links,
                                             User user,
                                             ServiceProvider sp,
                                             Map<Long, Consumer> oldSnapshot) {
        ...
        if (anomalyTypeId != null &&
            ...
            boolean anyChangeDetected = false;

            for (ConsumerAnomaly ca : links) {
                Consumer newC = ca.getConsumer();
                if (newC == null) continue;

                Consumer oldC = oldSnapshot.get(newC.getId());

                // 🔥 Detect if anomaly-triggering fields changed
                if (oldC != null && !isAnomalyResolved(oldC, newC)) {
                    anyChangeDetected = true;
                }

                String statusText = (newC.getStatus() == null) ? "" : newC.getStatus().trim();
                boolean accepted = statusText.equalsIgnoreCase("Accepted");
                boolean recycled = statusText.equalsIgnoreCase("Recycled");
                boolean consistent = Boolean.TRUE.equals(newC.getIsConsistent());

                if (recycled) recycledCount++;

                if (accepted) {
                    if (consistent) acceptedConsistent++;
                    else acceptedInconsistent++;
                }
            }

            log.info("computeGroupStatus anomaly={} changed={} recyc={} accInc={} accCons={}",
                    anomaly.getId(), anyChangeDetected, recycledCount, acceptedInconsistent, acceptedConsistent);


            // =============================================================
            // 1) ⭐ ALL RECYCLED BUT NO CHANGE → KEEP AS RE REPORTED
            // =============================================================
            if (recycledCount == total && !anyChangeDetected) {
                return AnomalyStatus.REPORTED; // 0
            }

            // =============================================================
            // 2) ⭐ ALL RECYCLED + CHANGE → WITHDRAWN
            // =============================================================
            if (recycledCount == total && anyChangeDetected) {
            ...
            // =============================================================
            // 3) ⭐ No improvement → still REPORTED
            // =============================================================
        ...
        // fallback to your existing rules for other anomaly types
    ...
    /**
     * Check if anomaly cause was resolved (ID, MSISDN).
     */
    private boolean isAnomalyResolved(Consumer oldC, Consumer newC) {
        ...
        // If all anomaly fields same → NOT resolved
        return msisdnSame && idSame;
    }

    /**
     * Deep clone only required anomaly fields.
     */
